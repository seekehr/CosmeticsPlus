...
class EventListener implements Listener
{
	...
	{
		if(isset(class_uses($event->getEntity())['seeker\morphs\Morph']) || isset(class_uses($event->getEntity())['seeker\entities\EntityTrait'])) {
			$event->setCancelled();
		}
	}
	...
	public function onSneak(PlayerToggleSneakEvent $event) : void
	{
		if($this->main->getDataProvider()->isRegisteredPlayer(($player = $event->getPlayer())->getName())){
			if(isset($this->main->morphs[strtolower($player->getName())]) && $this->main->getDataProvider()->canSeeMorphs($player)){
				...
				if(isset(class_uses($entity)['seeker\morphs\Morph'])){
